...
function addNotification(message, type = "info", data = {}) {
  ...
  notifs.unshift({ id: "n-" + Date.now(), message, type, data, read: false, date: new Date().toISOString() });
...
function getStoredProcurement() {
  try {
    return JSON.parse(localStorage.getItem("krishak-mitra-procurement") || "{}");
  } catch {
    return {};
  }
}
...
export const api = {
  ...
  createCentre: async (payload) => {
    ...
    addNotification("", "centrePublished", { name: newCentre.name, district: newCentre.district });
    return newCentre;
  },
  ...
  createBooking: async (payload) => {
    ...
    addNotification("", "bookingCreated", { farmer: newBooking.booking.farmer_name || "Farmer", slot: slotTime });
    return newBooking;
  },
  ...
  updateBookingStatus: async (bookingId, status) => {
    ...
    if (idx > -1) {
      ...
      saveBookings(bookings);
      const statusMap = { BOOKED: "BOOKED", CHECKED_IN: "CHECKED_IN", WAITING: "WAITING", WEIGHING: "WEIGHING", QUALITY_CHECK: "QUALITY_CHECK", ACCEPTED: "ACCEPTED", PAID: "PAID" };
      ...
      records[bookingId] = {
        ...
        actual_quantity: records[bookingId]?.actual_quantity || bookings[idx].booking.estimated_quantity,
        quality_grade: records[bookingId]?.quality_grade || "Pending",
        updated_at: new Date().toISOString(),
      };
      saveProcurement(records);
      addNotification("", "bookingStatus", { farmer: bookings[idx].booking.farmer_name || "Farmer", status });
    }
  ...
  requestPayment: async (bookingId, mobile, bank) => {
    ...
    if (idx > -1) {
      ...
      updatedBooking = bookings[idx].booking;
      saveBookings(bookings);
      addNotification("", "paymentRequested", { farmer: bookings[idx].booking.farmer_name || "Farmer" });
    }
    return updatedBooking;
  },
  ...
  getProcurement: async (bookingId) => {
    ...
    const booking = getStoredBookings().find((item) => item.booking.id === bookingId)?.booking;
    const record = {
      id: `p-${bookingId}`,
      booking_id: bookingId,
