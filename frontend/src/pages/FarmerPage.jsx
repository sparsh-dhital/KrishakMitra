...
export default function FarmerPage({
      ...
      try {
        ...
        setSelectedCrops(
          booking?.booking?.crops
            ? booking.booking.crops.map((c) => c.crop_id)
            : cropData?.[0]
              ? [cropData[0].id]
              : [],
        );
        if (booking?.booking?.crops) {
          ...
          booking.booking.crops.forEach(
            (c) => (initialQuantities[c.crop_id] = c.quantity),
          );
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (pRes.status === "fulfilled") {
        ...
        if (record?.id)
          api
            .getPayment(record.id, bookingId)
      ...
      if (bRes.status === "fulfilled" && bRes.value) {
        setBooking((prev) => ({
          ...prev,
          booking: { ...prev.booking, ...bRes.value },
        }));
      }
    });
    ...
    try {
      ...
      const newBooking = {
        ...
        booking: {
          ...
          farmer_mobile: paymentForm.mobile,
          farmer_bank: paymentForm.bank,
        },
      };
  ...
  async function createAuction(event) {
    ...
    try {
      ...
      const selectedCropRecord = crops.find(
        (crop) => crop?.id === targetCropId,
      );
      const cropNameToUse = useManualCrop
        ? manualCropName
        : selectedCropRecord?.name;
      const listing = await createAuctionDirectly({
  ...
  const journeySteps = [
    ...
    {
      title: "Quality",
      subtitle: procurement ? procurement.quality_grade || "Graded" : "Pending",
    },
    {
      title: "Acceptance",
      subtitle:
        procurement?.procurement_status === "ACCEPTED" ? "Accepted" : "Pending",
    },
    { title: "Payment", subtitle: payment ? `${payment.amount}` : "Pending" },
  ];

  const activeCrop = crops.find(
    (c) => c.id === (booking?.booking?.crops?.[0]?.crop_id || selectedCrops[0]),
  );
  ...
  const availableCrops = crops.filter((c) => {
    ...
    if (
      !centre ||
      !centre.supported_crops ||
      centre.supported_crops.length === 0
    )
    ...
    >
        ...
        <div className="grid min-w-0 grid-cols-1 gap-6 lg:grid-cols-3 max-w-6xl mx-auto">
          <div className="min-w-0 space-y-6 lg:col-span-2">
            <div className="mb-6">
              ...
              <h1 className="font-display text-4xl font-extrabold text-forest mt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                ...
                className="text-left rounded-2xl border border-line bg-surface p-4 hover:border-brand/40 hover:shadow-sm transition-all"
              >
                <p className="text-xs font-bold uppercase tracking-widest text-muted">
                  {t("apmcWorkflow") || "APMC workflow"}
                </p>
                <p className="font-bold text-forest mt-1">
                  {t("bookMandiSlot") || "Book a mandi slot"}
                </p>
                <p className="text-xs text-muted mt-1">
                  {t("chooseCentreTimeQuantity") ||
                    "Choose a centre, time, and quantity."}
                </p>
              </button>
              <button
              ...
              >
                ...
                <p className="font-bold text-forest mt-1">
                  {t("sellToDirectBuyers") || "Sell to direct buyers"}
                </p>
                <p className="text-xs text-muted mt-1">
                  {t("postCropReceiveBids") ||
                    "Post your crop and receive competing bids."}
                </p>
              </button>
            </div>
            ...
            <div className="bg-surface border border-line rounded-3xl p-4 sm:p-6 flex flex-col gap-6 shadow-sm sm:flex-row">
              <div className="min-w-0 flex-1">
                <Eyebrow className="mb-4">
                  {booking ? t("bookingConfirmed") : t("nextStep")}
                </Eyebrow>
                {booking ? (
                  <>
                    <div className="flex items-center gap-4 mb-6">
                      <div
                        className="p-3 bg-white rounded-xl shadow-sm border border-line cursor-pointer"
                        onClick={() => setShowQRModal(true)}
                      >
                        {booking?.token?.token_number && (
                          <QRCode
                            value={JSON.stringify({
                              ticketId: booking?.booking?.id,
                              tokenNumber: booking?.token?.token_number,
                              farmerName:
                                farmerName ||
                                booking?.booking?.farmer_name ||
                                "Unknown",
                              crops: (booking?.booking?.crops || []).map(
                                (c) => ({
                                  name: c.crop_name,
                                  quantity: c.quantity,
                                }),
                              ),
                              centerId: booking?.booking?.centre_id,
                              centreName:
                                centres.find(
                                  (c) => c.id === booking?.booking?.centre_id,
                                )?.name || "Unknown",
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-muted">
                          {t("yourToken")}{" "}
                          <span className="text-forest font-extrabold text-xl">
                            #{booking?.token?.token_number || "N/A"}
                          </span>
                        </p>
                        <p className="text-sm font-medium text-forest">
                          {activeCrop?.name} &bull;{" "}
                          {booking?.booking?.estimated_quantity || 0} q
                        </p>
                      </div>
                    </div>
                    <div className="space-y-3 border-t border-line pt-4">
                      ...
                      <div className="flex items-center gap-3 text-sm text-forest font-medium">
                        ...
                        {booking?.booking?.date || "TBD"}
                      </div>
                    </div>
                    <Button
                      className="w-full mt-4 gap-2 bg-emerald-700 hover:bg-emerald-800"
                      onClick={() => setShowQRModal(true)}
                    >
                      <QrCode className="w-4 h-4" /> View Gate Pass
                    </Button>
                    <Button
                    ...
                    >
                      {t("yourToken")} <ChevronRight className="w-4 h-4" />
                    </Button>
                  </>
                ) : (
                ...
                <div className="flex items-center justify-between w-full mt-2">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-bold text-forest">
                      {liveQueueStats.farmers_in_queue} {t("farmers")}
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-muted font-bold">
            ...
            <Card>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-forest text-sm">
                    {centres.find((c) => c.id === selectedCentre)?.name ||
                      "Unknown Centre"}
                  </p>
              ...
              >
                {t("centre")} <ChevronRight className="w-4 h-4 ml-1" />
              </Button>
            </Card>
          </div>
            ...
            <Card className="bg-forest text-white border-transparent shadow-xl">
              ...
              {queueEntry ? (
                <>
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-display text-2xl font-bold">
                        ~{queueEntry.estimated_wait_time} min
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="mb-6 p-4 bg-white/10 rounded-xl flex items-center justify-between">
                      <div>
                        ...
                        <p className="font-bold text-lg">
                          {booking.booking.allocated_time_minutes} min
                        </p>
                      </div>
                      <Button
                        variant="outline"
                        ...
                        size="sm"
                        onClick={() => toast.success("More time requested!")}
                      >
                        Request More
                      </Button>
                    </div>
                  ...
                  <Button
                    className="w-full bg-white text-forest hover:bg-slate-100"
                    onClick={() => setActiveTab("queue")}
                  >
                    {t("viewQueue")} <ChevronRight className="w-4 h-4 ml-1" />
                  </Button>
                </>
              ) : (
                <p className="text-sm font-medium text-white/70 py-4">
                  {t("nextStep")}
                </p>
              )}
            </Card>
      ...
      {activeTab === "centres" && (
        <div className="min-w-0 max-w-4xl mx-auto space-y-4">
          ...
          <h1 className="font-display text-4xl font-extrabold text-forest mb-8">
            ...
            >
              <div className="flex min-w-0 items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="break-words text-xs text-muted font-medium mt-1">
                    {c.district} &bull; Capacity: {c.daily_capacity} q/day
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <Button
                  variant={selectedCentre === c.id ? "primary" : "outline"}
                  size="sm"
                  onClick={() => {
          ...
          {booking && !showNewBookingForm ? (
            <Card className="text-center py-12 px-6">
              ...
              <p className="text-muted mb-8 max-w-md mx-auto">
                You already have a confirmed booking at{" "}
                {centres.find((c) => c.id === booking?.booking?.centre_id)
                  ?.name || "a centre"}
                . Creating a new booking will override your current one.
              </p>
            ...
            <Card className="min-w-0 overflow-hidden p-0 shadow-lg">
              <div className="scrollbar-hide flex items-center gap-4 overflow-x-auto border-b border-line bg-slate-50/50 p-4 sm:p-6">
                {["Centre", "Crop", "Quantity", "Slot", "Confirm"].map(
                  (step, idx) => {
                    ...
                    const canClick =
                      idx <= bookingStep ||
                      (idx === bookingStep + 1 &&
                        ((bookingStep === 0 && selectedCentre) ||
                          (bookingStep === 1 && selectedCrops.length > 0) ||
                          (bookingStep === 2 &&
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`w-6 h-6 rounded-full flex items-center justify-center text-xs text-white transition-colors ${isActive ? "bg-brand shadow-md" : isCompleted ? "bg-forest" : "bg-slate-300"}`}
                        >
                          {idx + 1}
                        </span>
                        {step}
                      ...
                      <div>
                        ...
                        <div className="space-y-4">
                          {selectedCrops.map((cropId) => {
                            ...
                            return (
                              <div
                                key={cropId}
                                className="flex items-center justify-between p-3 bg-white border border-line rounded-xl"
                              >
                                <span className="text-sm font-bold text-forest">
                                  {crop?.name}
                        ...
                        <div className="mt-8 flex justify-between">
                          ...
                          <Button
                            ...
                            disabled={selectedCrops.some(
                              (id) =>
                                !quantities[id] || Number(quantities[id]) <= 0,
                            )}
                          >
                            Next step
                          </Button>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-2">
                          {slots.length > 0 ? (
                              ...
                              return (
                                ...
                                >
                                  <div>
                                    ...
                                    <p className="text-xs text-muted font-medium mt-1">
                                      {slot.remaining} q available
                                    </p>
                                  </div>
                                  <Badge
                                    tone={isAvailable ? "success" : "default"}
                                  >
                            ...
                            <p className="text-muted py-4 col-span-2 text-center">
                      ...
                      <div>
                        ...
                        <div className="bg-slate-50 rounded-xl p-6 space-y-4 border border-line">
                          <div className="flex justify-between border-b border-line pb-4">
                            ...
                            <span className="font-bold text-forest">
                              {centres.find((c) => c.id === selectedCentre)
                                ?.name || "N/A"}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-line pb-4">
                        ...
                        <div className="flex flex-col items-stretch gap-3 pt-6 mt-4 sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <Button
                            ...
                            disabled={
                              !selectedSlot || selectedCrops.length === 0
                            }
                          >
                            {t("confirmBooking")}{" "}
      ...
      {activeTab === "token" && (
        ...
        >
          <div className="flex justify-between items-center mb-6">
            ...
            <h1 className="font-display text-xl sm:text-2xl font-bold text-forest text-center flex-1">
              Digital Token
            </h1>
          ...
          {booking?.token?.token_number ? (
            <div className="relative overflow-hidden bg-white rounded-3xl border border-line shadow-2xl drop-shadow-xl">
              <div className="bg-forest p-6 text-white text-center relative overflow-hidden">
                ...
                <p className="text-xs font-bold text-brand uppercase tracking-widest mb-1 relative z-10">
                  Confirmed Booking
                </p>
                <h2 className="font-display text-3xl font-extrabold relative z-10">
                  {booking?.booking?.date || "TBD"}
                </h2>
              </div>
              ...
              <div className="p-8 flex flex-col items-center text-center">
                <div className="bg-white p-6 rounded-2xl border-2 border-brand/20 shadow-inner flex flex-col items-center gap-4">
                  <QRCode
                    value={JSON.stringify({
                      ...
                      tokenNumber: booking.token.token_number,
                      farmerName:
                        farmerName || booking.booking?.farmer_name || "Unknown",
                      crops: (booking.booking?.crops || []).map((c) => ({
                        name: c.crop_name,
                        quantity: c.quantity,
                      })),
                      totalQuantity: booking.booking?.estimated_quantity || 0,
                      centerId: booking.booking?.centre_id,
                      centreName:
                        centres.find((c) => c.id === booking.booking?.centre_id)
                          ?.name || "Unknown",
                      slotTime: booking.booking?.slot_time || "N/A",
                      date:
                        booking.booking?.date ||
                        new Date().toISOString().split("T")[0],
                      status: booking.booking?.status || "BOOKED",
                      estimatedFare: booking.booking?.estimated_fare || 0,
                    })}
                    size={200}
                    level="H"
                ...
                <Button
                  className="w-full mt-6 gap-2 bg-harvest text-forest hover:bg-harvest-hover"
                  onClick={() => setShowQRModal(true)}
                >
            ...
            <Card className="flex flex-col items-center gap-6 text-center py-12">
              ...
              <div>
                <p className="text-forest font-bold text-lg mb-1">
                  No token yet
                </p>
                <p className="text-muted font-medium">
                  Book a slot to generate your gate pass.
                </p>
              </div>
              <Button onClick={() => setActiveTab("bookings")} className="mt-2">
                Book a Slot
              </Button>
            </Card>
          )}
        ...
        <div className="min-w-0 max-w-2xl mx-auto">
          ...
          {queueEntry ? (
            <Card className="bg-forest text-white border-transparent text-center">
              ...
              <p className="font-display text-7xl font-extrabold mb-4">
                #{queueEntry.queue_position}
              </p>
          ...
          {booking?.booking ? (
            <Card>
              <div className="space-y-6">
                  ...
                  booking.booking.receipt_url && (
                    <div className="pt-4 mt-4 border-t border-line">
                      ...
                      {booking.booking.receipt_url.startsWith("data:image") ? (
                        <img
                          src={booking.booking.receipt_url}
                          alt="Receipt"
                          className="max-w-full rounded-xl border border-line"
                        />
                      ) : (
                        <a
                          href={booking.booking.receipt_url}
                          target="_blank"
                          rel="noreferrer"
                          className="text-brand hover:underline font-bold text-sm"
                        >
                ...
                {["QUALITY_CHECK", "WEIGHING", "ACCEPTED"].includes(
                ...
                {booking.booking.status === "PAYMENT_REQUESTED" && (
                  <div className="pt-4 border-t border-line text-center text-sm font-medium text-amber-600 bg-amber-50 p-4 rounded-xl border-amber-100">
                    Payment request submitted. Awaiting processing by the
                    administrator.
                  </div>
                )}
              </div>
            ...
            <Card className="text-center py-10">
              <p className="text-muted font-medium">
                Payment details will appear after procurement is accepted.
              </p>
            </Card>
          )}
        </div>
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <div>
            ...
            <p className="text-sm text-muted mt-1">
              Create a listing alongside your existing APMC mandi bookings.
            </p>
          </div>
          <div className="flex gap-2 border-b border-line">
            <button
              className={`px-4 py-3 text-sm font-bold border-b-2 ${auctionTab === "listings" ? "border-brand text-brand" : "border-transparent text-muted"}`}
              onClick={() => setAuctionTab("listings")}
            >
              My listings
            </button>
            <button
              className={`px-4 py-3 text-sm font-bold border-b-2 ${auctionTab === "create" ? "border-brand text-brand" : "border-transparent text-muted"}`}
          ...
          {auctionTab === "create" ? (
            <Card className="max-w-2xl">
              <h2 className="font-display text-xl font-bold text-forest mb-6">
                New private listing
              </h2>
              <form onSubmit={createAuction} className="space-y-5">
                <div>
                  ...
                  {!useManualCrop ? (
                    <div className="space-y-3">
                      <Select
                        value={auctionCrop}
                        onChange={(event) => setAuctionCrop(event.target.value)}
                        required
                      >
                        {crops.map((crop) => (
                    ...
                    <div className="space-y-3">
                      <Input
                        ...
                        onChange={(event) =>
                          setManualCropName(event.target.value)
                        }
                ...
                <div>
                  ...
                  <Select
                    value={auctionGrade}
                    onChange={(event) => setAuctionGrade(event.target.value)}
                    required
                  >
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setAuctionBasePrice(event.target.value)
                    }
                ...
                <Button
                  ...
                  disabled={
                    auctionSaving ||
                    (!auctionCrop && !crops[0]?.id && !useManualCrop) ||
                    (useManualCrop && !manualCropName)
                  }
                >
                  {auctionSaving ? "Publishing..." : "Publish private listing"}
                </Button>
              </form>
            </Card>
          ) : auctions.length ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {auctions.map((auction) => (
                <AuctionCard
                  ...
                  onRemoved={(auctionId) =>
                    setAuctions((current) =>
                      current.filter((item) => item?.id !== auctionId),
                    )
                  }
                />
              ))}
            ...
            <Card className="text-center py-10">
              <p className="text-muted font-medium">
                You have no private market listings yet.
              </p>
              <Button className="mt-4" onClick={() => setAuctionTab("create")}>
            ...
            {bidNotifications.length > 0 || notifications.length > 0 ? (
              <div className="space-y-4 mb-4">
                ...
                {notifications.map((n) => (
                  ...
                  >
                    <span
                      className={`w-2 h-2 mt-1.5 rounded-full shrink-0 ${n.read ? "bg-slate-300" : "bg-brand"}`}
                    />
                    <div>
                      <p className="text-sm font-bold text-forest">
              ...
              <p className="text-muted font-medium text-center py-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-line bg-slate-50">
              ...
              <p className="text-sm text-muted">
                Please provide your mobile number and bank account details for
                payment.
              </p>
            </div>
            <form onSubmit={handleRequestPayment} className="p-6 space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setPaymentForm({ ...paymentForm, mobile: e.target.value })
                  }
                  required
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setPaymentForm({ ...paymentForm, bank: e.target.value })
                  }
                  required
                  placeholder="E.g., SBI A/C 1234..."
                />
              </div>
      ...
      <QRCodeModal
        ...
        slotData={{
          ...
          centreName:
            centres.find((c) => c.id === booking?.booking?.centre_id)?.name ||
            "Unknown Centre",
        }}
      />
    </SidebarLayout>
