  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
<<<<<<< HEAD
          <p className="text-xs font-bold uppercase tracking-widest text-brand mb-2">{t("privateMarket")}</p>
          <h1 className="font-display text-2xl font-bold text-forest">{t("directMarketplace")}</h1>
          <p className="text-sm text-muted mt-1">{t("marketplaceIntro")}</p>
        </div>
        <Button variant="outline" size="sm" onClick={loadAuctions} disabled={loading} className="gap-2"><RefreshCw className="w-4 h-4" /> {t("refresh")}</Button>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Card className="p-4"><p className="text-xs font-bold uppercase tracking-widest text-muted">{t("openListings")}</p><p className="font-display text-2xl font-extrabold text-forest mt-1">{auctions.length}</p></Card>
        <Card className="p-4"><p className="text-xs font-bold uppercase tracking-widest text-muted">{t("lowestPrice")}</p><p className="font-display text-2xl font-extrabold text-brand mt-1">{auctions.length ? `₹${Math.min(...auctions.map((auction) => Number(auction?.base_price || 0))).toLocaleString("en-IN")}` : "-"}</p></Card>
        <Card className="p-4"><p className="text-xs font-bold uppercase tracking-widest text-muted">{t("directTrade")}</p><p className="font-display text-2xl font-extrabold text-forest mt-1 flex items-center gap-2">{t("live")} <Sparkles className="w-5 h-5 text-amber-500" /></p></Card>
      </div>

      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1"><Search className="w-4 h-4 text-muted absolute left-4 top-1/2 -translate-y-1/2" /><Input className="pl-11" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("searchCrops")} /></div>
        <div className="relative sm:w-56"><SlidersHorizontal className="w-4 h-4 text-muted absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none" /><select value={sort} onChange={(event) => setSort(event.target.value)} className="h-12 w-full appearance-none rounded-xl border border-line bg-surface pl-11 pr-4 text-sm font-bold text-forest"><option value="newest">{t("newest")}</option><option value="price">{t("lowestPrice")}</option><option value="quantity">{t("largestQuantity")}</option></select></div>
      </div>

      {loading ? <Card><p className="text-sm text-muted">{t("loadingListings")}</p></Card> : auctions.length === 0 ? (
        <Card className="text-center py-12"><Store className="w-10 h-10 text-brand mx-auto mb-4" /><h2 className="font-bold text-forest">{t("noListings")}</h2><p className="text-sm text-muted mt-2">{t("newListings")}</p></Card>
      ) : (
        visibleAuctions.length ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">{visibleAuctions.map((auction) => <AuctionCard key={auction?.id} auction={auction} role={role} buyerId={buyerId} onBid={(item, current) => { setSelectedAuction(item); setBid(String(Number(current || 0) + 1)); }} />)}</div> : <Card className="text-center py-10"><p className="text-muted font-medium">{t("noMatch")}</p></Card>
=======
...
<<<<<<< HEAD
            <div className="flex items-start justify-between gap-4 mb-6"><div><Badge tone="success">{t("openAuction")}</Badge><h2 className="font-display text-xl font-bold text-forest mt-2">{t("higherBid")}</h2><p className="text-sm text-muted mt-1">{selectedAuction?.crops?.name || t("crop")}</p></div><button className="text-muted hover:text-forest" onClick={() => setSelectedAuction(null)} aria-label={t("closed")}><X className="w-5 h-5" /></button></div>
            <form onSubmit={submitBid} className="space-y-5"><div><label className="block text-sm font-bold text-forest mb-2">{t("offerPerQuintal")}</label><Input type="number" min="0" step="0.01" value={bid} onChange={(event) => setBid(event.target.value)} required /></div><Button type="submit" className="w-full" disabled={submitting}>{submitting ? t("submitting") : t("submitBid")}</Button></form>
=======
