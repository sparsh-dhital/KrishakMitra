      ...
      {steps.map((step, idx) => {
        ...
        return (
          <div key={idx} className="relative flex-1 flex flex-col items-center">
            {idx !== 0 && (
              <div className="absolute top-4 -left-1/2 w-full h-0.75 bg-slate-100 z-0">
                <div
...
export function SidebarLayout({
  ...
  return (
    <div className="flex min-h-screen min-w-0 overflow-x-hidden bg-cream">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <p className="text-sm text-white font-bold truncate max-w-37.5">
                {displayName}
              </p>
            </div>
          </div>
      ...
      <main
        className={cn(
          "min-w-0 flex-1 pb-24 lg:pb-0 transition-all duration-300 ease-in-out",
          isExpanded ? "lg:pl-65" : "lg:pl-18",
        )}
      >
        <header className="h-18 border-b border-line/50 bg-white/70 backdrop-blur-2xl sticky top-0 z-40">
          <div className="flex h-full items-center gap-4 px-4 sm:px-6 lg:px-8">
            ...
            <div className="ml-auto flex items-center gap-2 sm:gap-4 shrink-0">
              ...
              <div className="flex items-center gap-3 pl-2 sm:pl-4 border-l border-slate-200">
                <div className="w-10 h-10 rounded-full bg-linear-to-tr from-brand to-emerald-400 flex items-center justify-center text-white text-sm font-extrabold shadow-md shrink-0 border border-white">
                  {displayName
                    .split(" ")
                    .map((w) => w[0])
                    .join("")
      ...
      {moreOpen && secondaryNavItems.length > 0 && (
        <div className="lg:hidden fixed inset-x-3 bottom-19 z-50 max-h-[min(70vh,28rem)] overflow-y-auto rounded-2xl border border-line bg-surface p-2 shadow-xl">
        ...
        {primaryNavItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="max-w-full wrap-break-word text-center text-[10px] leading-tight font-bold">
                {item.label}
              </span>
            </button>
          );
