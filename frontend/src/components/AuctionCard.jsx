import { useEffect, useState } from "react";
import { useTranslation } from "react-i18next";
import toast from "react-hot-toast";
import { motion, AnimatePresence } from "framer-motion";
import { Gavel, Package, IndianRupee, CheckCircle2, Clock3, TrendingUp, Trash2 } from "lucide-react";
import { Badge, Button, Card } from "./ui";
import { acceptHighestBidDirectly, getAuctionBids, getHighestBid, removeAuctionDirectly, supabase } from "../services/biddingService";

const money = (value) => `${Number(value || 0).toLocaleString("en-IN")}`;

export function AuctionCard({ auction, role, buyerId, onBid, onUpdated, onRemoved }) {
  const { t } = useTranslation();
  const [highestBid, setHighestBid] = useState(null);
  const [bidHistory, setBidHistory] = useState([]);
  const [timeLeft, setTimeLeft] = useState("");
  const [accepting, setAccepting] = useState(false);
  const [removing, setRemoving] = useState(false);
  const auctionId = auction?.id;

  useEffect(() => {
    if (!auctionId) return undefined;
    let active = true;

    Promise.all([getHighestBid(auctionId), getAuctionBids(auctionId)]).then(([bid, bids]) => {
      if (!active) return;
      setHighestBid(bid);
      setBidHistory(bids || []);
    }).catch(() => {});

    // No realtime without supabase  still need to return a valid cleanup
    if (!supabase) {
      return () => { active = false; };
    }

    const channel = supabase
      .channel(`auction-${auctionId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "bids", filter: `auction_id=eq.${auctionId}` }, (payload) => {
        setHighestBid((current) => Number(payload?.new?.offered_price || 0) > Number(current?.offered_price || 0) ? payload.new : current);
        setBidHistory((current) => [payload.new, ...current].sort((a, b) => Number(b.offered_price) - Number(a.offered_price)));
      })
      .subscribe();

    return () => {
      active = false;
      supabase.removeChannel(channel);
    };
  }, [auctionId]);

  useEffect(() => {
    const expiry = auction?.expires_at ? new Date(auction.expires_at).getTime() : 0;
    if (!expiry) {
      setTimeLeft(t("auction.openEnded"));
      return undefined;
    }
    const update = () => {
      const remaining = Math.max(0, expiry - Date.now());
      const hours = Math.floor(remaining / 3600000);
      const minutes = Math.floor((remaining % 3600000) / 60000);
      const seconds = Math.floor((remaining % 60000) / 1000);
      setTimeLeft(remaining ? `${hours}h ${minutes}m ${seconds}s` : t("auction.ended"));
    };
    update();
    const timer = window.setInterval(update, 1000);
    return () => window.clearInterval(timer);
  }, [auction?.expires_at]);

  async function acceptBid() {
    setAccepting(true);
    try {
      await acceptHighestBidDirectly(auctionId);
      toast.success(t("auction.topBidAccepted"));
      onUpdated?.(auctionId);
    } catch (error) {
      toast.error(error?.message || t("auction.acceptFailed"));
    } finally {
      setAccepting(false);
    }
  }

  async function removeAuction() {
    if (!window.confirm(t("auction.removeConfirm"))) return;
    setRemoving(true);
    try {
      await removeAuctionDirectly(auctionId);
      toast.success(t("auction.removed"));
      onRemoved?.(auctionId);
    } catch (error) {
      toast.error(error?.message || t("auction.removeFailed"));
    } finally {
      setRemoving(false);
    }
  }

  const cropName = auction?.crops?.name || auction?.crop_name || t("marketplace.cropListing");
  const quantity = auction?.quantity ?? auction?.quantity_quintals ?? 0;
  const currentPrice = highestBid?.offered_price || auction?.base_price || 0;

  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      whileHover={{ scale: 1.01, boxShadow: "0 8px 30px rgba(0,0,0,0.09)" }}
      transition={{ duration: 0.2 }}
      className="bg-surface border border-line rounded-3xl p-6 shadow-sm"
    >
      <div className="flex flex-col gap-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Gavel className="w-5 h-5 text-brand" />
              <h3 className="font-display text-lg font-bold text-forest">{cropName}</h3>
            </div>
            <Badge tone={auction?.status === "open" ? "success" : "default"}>{auction?.status === "open" ? t("marketplace.open") : auction?.status || t("marketplace.open")}</Badge>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold text-muted uppercase tracking-widest">{t("auction.currentHighest")}</p>
            <AnimatePresence mode="wait">
              <motion.p
                key={currentPrice}
                initial={{ y: -10, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                className="font-display text-2xl font-extrabold text-brand"
              >
                {money(currentPrice)}
              </motion.p>
            </AnimatePresence>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 border-y border-line py-4">
          <div className="flex items-center gap-2 text-sm text-muted"><Package className="w-4 h-4" /> {quantity} {t("auction.quintalsShort")}</div>
          <div className="flex items-center gap-2 text-sm text-muted"><IndianRupee className="w-4 h-4" /> {t("auction.base")} {money(auction?.base_price)}</div>
        </div>

        <div className="flex items-center justify-between gap-3 text-xs font-bold">
          <span className="flex items-center gap-1.5 text-amber-700"><Clock3 className="w-4 h-4" /> {timeLeft}</span>
          <span className="flex items-center gap-1.5 text-brand">
            {auction?.status === "open" && (
              <span className="relative flex h-2 w-2 mr-1">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-brand"></span>
              </span>
            )}
            <TrendingUp className="w-4 h-4" /> {bidHistory.length} {t(bidHistory.length === 1 ? "auction.bid" : "auction.bids")}
          </span>
        </div>

        {bidHistory.length > 0 && (
          <div className="rounded-xl bg-slate-50 border border-line px-3 py-2 text-xs text-muted">
            {t("auction.latestBidBy")} <span className="font-bold text-forest">{bidHistory[0]?.buyer_id || t("buyer.role")}</span> {t("auction.at")} <span className="font-bold text-brand">{money(bidHistory[0]?.offered_price)}</span>
          </div>
        )}

        {role === "farmer" ? (
          <div className="flex flex-col sm:flex-row gap-2">
            <Button variant="dark" className="flex-1 gap-2" onClick={acceptBid} disabled={accepting || removing || !highestBid || auction?.status !== "open"}>
              <CheckCircle2 className="w-4 h-4" /> 
              {auction?.status === "awarded" ? t("auction.bidAccepted") : (accepting ? t("auction.accepting") : (!highestBid ? t("auction.waitingForBids") : t("auction.acceptTopBid")))}
            </Button>
            <Button variant="outline" className="gap-2 border-red-200 text-red-600 hover:border-red-300 hover:bg-red-50" onClick={removeAuction} disabled={accepting || removing || auction?.status !== "open"}>
              <Trash2 className="w-4 h-4" /> {removing ? t("auction.removing") : t("auction.remove")}
            </Button>
          </div>
        ) : (
          <Button className="w-full" onClick={() => onBid?.(auction, currentPrice)}>{t("marketplace.placeHigherBid")}</Button>
        )}
      </div>
    </motion.div>
  );
}

export default AuctionCard;
